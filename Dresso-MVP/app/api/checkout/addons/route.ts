...
export async function POST(req: NextRequest) {
  const { listingId, addons } = await req.json() as { listingId: string, addons: ("bump"|"highlight")[] };
  if (!listingId || !addons?.length) return NextResponse.json({ error: "listingId och addons krävs" }, { status: 400 });
